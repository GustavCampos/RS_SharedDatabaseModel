from sqlalchemy import Column, BigInteger, Integer, String, ForeignKey, Enum, TIMESTAMP, Sequence
...
class Client(Base):
    ...
    id = Column(BigInteger, Sequence("client_id_seq", start=1, increment=1),primary_key=True)
    cpf = Column(String, nullable=False, unique=True)
    complete_name = Column(String, nullable=False)
    version = Column(Integer, nullable=False, default=1)
    
    # Client -< BankAccount
    bank_accounts = relationship("BankAccount", back_populates="owner_obj", cascade="all, delete-orphan")
            
class BankAccount(Base):
    __tablename__ = "bank_accounts"
    
    id = Column(BigInteger, Sequence("bkacc_id_seq", start=1, increment=1),primary_key=True)
...
class Transaction(Base):
    ...
    id = Column(BigInteger, Sequence("transaction_id_seq", start=1, increment=1), primary_key=True)
